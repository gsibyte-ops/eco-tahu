...
<div class="max-w-2xl mx-auto bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
    <h2 class="text-xl font-bold text-gray-800 mb-6">Form Edit Kategori</h2>
    ...
    <form action="{{ route('admin.kategori.update', $kategori->id) }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <select name="tipe" required
                    class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                <option value="produk_tahu" {{ old('tipe', $kategori->tipe) == 'produk_tahu' ? 'selected' : '' }}>Produk Tahu</option>
                <option value="limbah" {{ old('tipe', $kategori->tipe) == 'limbah' ? 'selected' : '' }}>Limbah (Ampas)</option>
            </select>
        </div>
