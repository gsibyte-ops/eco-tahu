            ...
            <div class="lg:col-span-2 space-y-4">
                @foreach ($cart as $key => $item)
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex gap-4" id="item-{{ $key }}">
                        <div class="w-20 h-20 rounded-2xl bg-gray-50 flex-shrink-0 overflow-hidden border border-gray-100">
                            @if ($item['gambar'])
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between items-start gap-2">
                                <div class="min-w-0">
                                    <span class="inline-block text-[10px] font-semibold uppercase tracking-wide {{ $item['type'] === 'produk' ? 'text-emerald-600 bg-emerald-50' : 'text-amber-600 bg-amber-50' }} px-2 py-1 rounded-full">
                                ...
                                <button type="button" onclick="removeItem('{{ $key }}')" class="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-red-50 text-gray-400 hover:text-red-500 transition flex-shrink-0">
                            ...
                            <div class="flex items-center justify-between mt-4">
                                <div class="flex items-center bg-gray-100 rounded-xl overflow-hidden border border-gray-200">
                                    <button type="button" onclick="updateQty('{{ $key }}', -1)" class="w-9 h-9 flex items-center justify-center text-gray-600 hover:text-emerald-600">−</button>
                                    <span class="w-10 text-center font-semibold text-sm" id="qty-{{ $key }}">{{ $item['qty'] }}</span>
                                    <button type="button" onclick="updateQty('{{ $key }}', 1)" class="w-9 h-9 flex items-center justify-center text-gray-600 hover:text-emerald-600">+</button>
                                </div>

                                <p class="font-bold text-gray-800" id="subtotal-{{ $key }}">Rp {{ number_format($item['harga'] * $item['qty'], 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                @endforeach
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sticky top-24">
                    ...
                    <div class="space-y-2 mb-4 pb-4 border-b border-gray-100">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-semibold text-gray-800" id="totalItems">{{ collect($cart)->sum('qty') }} item</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">Subtotal</span>
                            <span class="font-semibold text-gray-800" id="subtotalDisplay">Rp {{ number_format($subtotal, 0, ',', '.') }}</span>
                        </div>
                    </div>
    ...
    @else
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-16 text-center max-w-md mx-auto">
            ...
            <div class="flex gap-3 justify-center">
                <a href="{{ route('user.produk.index') }}" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-xl transition">Belanja Tahu</a>
                <a href="{{ route('user.limbah.index') }}" class="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold rounded-xl transition">Belanja Limbah</a>
            </div>
        </div>
    @endif
</div>
